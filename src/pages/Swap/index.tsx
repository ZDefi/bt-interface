...
import { INITIAL_ALLOWED_SLIPPAGE, SELECT_COIN_ADDRESS, DEFAULT_MIN_SLIPPAGE } from 'constants/index'
...
const StyledTrade = styled.div`
  ...
  background: linear-gradient(90deg, #ff7750 0%, rgba(239, 196, 67, 0.61) 100%);
...
const Swap = () => {
  ...
  const urlLoadedTokens: Token[] = useMemo(
    () => [loadedInputCurrency, loadedOutputCurrency]?.filter((c): c is Token => c instanceof Token && c.address !== SELECT_COIN_ADDRESS) ?? [],
  ...
  const allowedSlippage = useMemo(() => {
    ...
    const tokenSlippage = DEFAULT_MIN_SLIPPAGE.find(f =>
      f.symbol === trade?.inputAmount.currency.symbol || f.symbol === trade?.outputAmount.currency.symbol)
    if (tokenSlippage) {
  ...
  return (
    <>
      ...
      {isSwapMining &&
        <StyledTrade>
          <Text color="#FFFFFF" mr="1rem">
            {t('Reward %reward% BEE', { reward: reward || '0.0000' })}
          </Text>
          <Button scale="sm" onClick={handleWithdrawTrade}>{t('Withdraw')}</Button>
        </StyledTrade>
      }
                ...
                <RowBetween>
                  ...
                  >
                    {approval === ApprovalState.PENDING ? (
                      ...
                      t(`Approve %symbol%`, { symbol: currencies[Field.INPUT]?.symbol })
                    )}
                  </Button>
                  <Button
                    onClick={() => {
