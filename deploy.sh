...
;;
"domain")
echo "发布到哪个域名，如:babyswap.io就输入io即可,测试环境输入test"
;;
"filename")
...
test=/root/baby
xyz=/root/baby
FILEPATH=/Users/zhangwenpei/chain/baby/baby-swap-interface
FILENAME=build.zip
...
function deploy_io() {
declare -a arry
arry=([1]="54.179.62.187" [2]="13.228.162.22")
for i in ${!arry[@]};do
...
declare -a arry
arry=([1]="18.140.173.100" [2]="18.139.178.118" [3]="54.251.210.67" [4]="52.220.6.201")
for i in ${!arry[@]};do
        backpath="/tmp/bakup/finance_`echo $RANDOM`"
        scp ${FILEPATH}/${FILENAME} root@${arry[$i]}:/tmp
        ssh root@${arry[$i]} "mkdir -p $backpath && mv $finance/$PROJECT/* $backpath"
        ssh root@${arry[$i]} "unzip /tmp/${FILENAME} -d $finance/$PROJECT/"
done
}
...
declare -a arry
arry=([1]="18.140.173.100" [2]="18.139.178.118")
for i in ${!arry[@]};do
...
function deploy_xyz() {
declare -a arry
arry=([1]="54.179.207.197" [2]="13.228.18.16")
for i in ${!arry[@]};do
